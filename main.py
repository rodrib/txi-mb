...
#st.write("Primeros registros del DataFrame con los cambios realizados:")
...
#st.write(tipos_de_datos)

# Encontrar el último valor de saldo que tenga "3" en la columna "Asiento"
#saldo_marzo = dataframe.loc[dataframe['Asiento'] == 3, 'saldo'].iloc[-1]

# Encontrar el último valor de débito que tenga "3" en la columna "Asiento"
#debito_marzo = dataframe.loc[dataframe['Asiento'] == 3, 'debito'].iloc[-1] if not dataframe.loc[dataframe['Asiento'] == 3, 'debito'].empty else 0

# Encontrar el último valor de crédito que tenga "3" en la columna "Asiento"
#credito_marzo = dataframe.loc[dataframe['Asiento'] == 3, 'credito'].iloc[-1] if not dataframe.loc[dataframe['Asiento'] == 3, 'credito'].empty else 0

import numpy as np

# Verificar si debito_marzo es NaN o 0, y asignarle 0 en ese caso
#if pd.isnull(debito_marzo) or debito_marzo == 0:
#    debito_marzo = 0.0



# Mostrar el débito y crédito de marzo
#st.write("Débito de marzo:", debito_marzo)
#st.write("Crédito de marzo:", credito_marzo)


# Mostrar el saldo de marzo
#st.write("Saldo de marzo:", saldo_marzo)

#choice = ui.select(options=["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"])

#st.markdown(f"Currrent value: {choice}")

#st.write("Mes Elegido:", choice)
...
# Crear DataFrame con la columna Fecha
# Leer el archivo CSV
nombre_archivo = 'taxi-all-23-3.csv'
...
df1["Fecha"] = pd.to_datetime(df1["Fecha"] + "/2023", format="%d/%m/%Y")
...
# Mostrar el DataFrame df_calendar en Streamlit con un título
st.subheader("Cantidad de Movimientos por fecha")
#st.write(df_calendar)


# Crear el gráfico de heatmap calendar
fig = calplot(df_calendar, x="Fecha", y="Count")
